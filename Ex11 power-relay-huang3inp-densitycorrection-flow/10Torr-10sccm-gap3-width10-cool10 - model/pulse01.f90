...
program test_co2
    ...
    double precision            :: 	time  = 0.0d0, time_end = 25.3d-2, dtime = 1.0d-15, & ! times, s
                                    powertime = 12.65d-2, cooltime = 25.3d-2, ptime = 0.0d0, &
    ...
    write(*,'(6(1pe12.4))') time, reduced_field

    open( 10, file = "data-conditions.txt" )
    open( 11, file = "data-maindens.txt" )

    write( 10, '(7a30)' ) "Time", "Reduced field", "Te(K)", "Te(eV)", "Electron density", "Power density", "Power"
    write( 10, '(7a30)' ) "s", "Td", "K", "eV", "cm^-3", "W/cm^3", "W"
    write( 11, '(9a30)' ) "Time", "CO2", "CO2(Va)", "CO2(Vb)", "CO2(V1)", "CO2(Vd)", "CO", "O2", "O"
    write( 11, '(9a30)' ) "s", "cm^-3", "cm^-3", "cm^-3", "cm^-3", "cm^-3", "cm^-3", "cm^-3", "cm^-3"
    write( 11, '(9a30)' ) " ", "ground", "Va", "Vb", "V1", "Vd", " ", " ", " "
    ...
    do while(time .lt. time_end)
        ...
        ! else if( ( ptime >= powertime ) .and. ( (ptime + n * powertime) < cooltime ) ) then 
        !     ! reduced_field = 0d0
        !     ptime = 0
        !     dtime = 1.0d-15
        !     n = n + 1
        ...
        if (ptime < powertime) then 
            Prf = 50
            Pin = Prf/((pi*(r_out**2 - r_in**2) * width * n_ring) * 1d6)
            reduced_field = sqrt(Pin / ( all_neutral * ele_charge * e_dens * ele_mobility_n ) ) * 1d17 
        ...
        ! correct the density 
        den1 = sum(density)
        pres1 = den1 * kb * gas_temperature * 1.0d6
        density = density / pres1 * pressure * torr2pa
        ...
        write(10, '(7(1pe30.8))' ) time, reduced_field, ele_temperature, ele_temperature/11605, e_dens, Pin, Prf
        ...
        ! call ZDPlasKin_timestep(time,dtime)
        write(*,'(4(1pe12.4))') time, ptime, dtime, reduced_field

        ! if ((ptime > 5.0d-12) .and. (ptime < 1.0d-10) ) then 
        !     dtime = 1.0d-14

        if ((ptime > 1.0d-10) .and. (ptime < 1.0d-8) ) then 
            dtime = 1.0d-12
        else if ((ptime > 1.0d-8) .and. (ptime < 1.0d-6) ) then 
            dtime = 1.0d-10
        else if ((ptime > 1.0d-6) .and. (ptime < 1.0d-4) ) then 
            dtime = 1.0d-8
        else if ( (ptime > 1.0d-4).and. (ptime < 1.0d-2) .and. (ptime < powertime) )  then 
            dtime = 1.0d-6
        else if ( (ptime > 1.0d-2).and. (ptime < powertime) )  then 
            dtime = 1.0d-4
        else if ( (ptime > powertime) .and. (ptime < (powertime + 1.0d-10)))  then 
            ! ptime = 0d0
            dtime = 1.0d-15
        else if ( (ptime > (powertime + 1.0d-10) ) .and. (ptime < (powertime + 1.0d-8)) ) then
            dtime = 1.0d-12
        else if ( (ptime > (powertime + 1.0d-8) ) .and. (ptime < (powertime + 1.0d-6)) ) then
            dtime = 1.0d-10
        else if ( (ptime > (powertime + 1.0d-6) ) .and. (ptime < (powertime + 1.0d-4)) ) then
            dtime = 1.0d-8
        else if ( (ptime > (powertime + 1.0d-4) ) .and. (ptime < (powertime + 1.0d-2)) .and. (ptime < cooltime))  then
            dtime = 1.0d-6
        else if ( (ptime > (powertime + 1.0d-2) ) .and. (ptime < cooltime) ) then
            dtime = 1.0d-4
        else
            ! ptime = 0d0
            dtime = 1.0d-15
